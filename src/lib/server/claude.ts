...
const BADGE = `\n\n---\n*Documentation generated by [Codec8](https://codec8.com) — AI-powered docs for GitHub repos.*`;

function appendBadge(content: string, docType: DocType): string {
	// Only append to readme and setup — API/architecture docs are more technical
	if (docType === 'readme' || docType === 'setup') {
		return content.trimEnd() + BADGE;
	}
	return content;
}
...
export async function generateDocumentation(
	...
	try {
		...
		return {
			type: docType,
			content: appendBadge(textContent.text, docType),
			success: true
		};
	} catch (error) {
